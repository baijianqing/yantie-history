...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .post-court-explorer.chapter-map-panel {
      grid-template-columns: minmax(280px, 0.82fr) minmax(360px, 1.18fr);
      border-color: rgba(243,196,109,0.28);
      background: linear-gradient(180deg, rgba(17,24,34,0.98), rgba(9,13,19,0.96));
    }

    .post-court-explorer.chapter-map-panel[hidden] {
      display: none;
    }
    ...
    .post-court-overview strong {
      color: #fff4d6;
      font-size: 15px;
    }

    .post-court-overview ul {
      display: grid;
      gap: 9px;
      margin: 0;
      padding-left: 18px;
    ...
    .external-echo-panel b {
      color: #f3c46d;
    }

    .external-echo-viewpoint {
      border-left: 3px solid rgba(104,166,188,0.56);
      padding-left: 9px;
    ...
    .material-guide-panel {
      ...
      margin: 0 10px 10px;
      padding: 12px;
      border: 1px solid rgba(183,205,133,0.42);
      border-radius: 8px;
      background: rgba(9,13,19,0.5);
      color: rgba(255,244,214,0.74);
      font-size: 12px;
    ...
    .material-guide-panel[hidden] {
      display: none;
    }

    .material-guide-panel > strong {
      color: #e6f5b8;
      font-size: 13px;
    }
    ...
    .material-guide-card {
      ...
      gap: 8px;
      padding: 10px;
      border: 1px solid rgba(183,205,133,0.26);
      border-radius: 8px;
      background: rgba(255,244,214,0.05);
    }

    .material-guide-card h4,
    ...
    .material-guide-section b {
      color: rgba(255,244,214,0.9);
      font-size: 11px;
    }

    .material-guide-section p {
      color: rgba(255,244,214,0.72);
      font-size: 12px;
      line-height: 1.55;
    }
    ...
    @media (max-width: 640px) {
      ...
      .stage.is-prologue-active .story-panel {
        ...
        border: 1px solid rgba(255,236,188,0.2);
        border-radius: 10px;
        background: linear-gradient(180deg, rgba(9,13,19,0.72), rgba(9,13,19,0.5));
      ...
      .stage.is-prologue-active .map-layer svg {
        position: absolute;
        left: 0;
        top: 102px;
        width: 100vw;
        height: clamp(260px, calc(100dvh - 400px), 444px);
        max-height: calc(100dvh - 400px);
      }

      .stage.is-prologue-active .court-pressure-opening {
        inset: 0;
      }
      ...
      .stage.is-prologue-active[data-experience-phase="pressure_entry"] .pressure-director-panel {
        top: auto;
      ...
      .stage.is-prologue-active[data-experience-phase="pressure_entry"] .pressure-director-panel h2,
      .stage.is-prologue-active[data-experience-phase="pressure_entry"] .pressure-director-panel p,
      .stage.is-prologue-active[data-experience-phase="pressure_entry"] .support-strength,
      .stage.is-prologue-active[data-experience-phase="pressure_entry"] .support-note {
        display: none;
      }

      .stage.is-prologue-active[data-experience-phase="pressure_entry"] .pressure-time {
        margin-bottom: 6px;
        font-size: 10px;
      }

      .stage.is-prologue-active[data-experience-phase="pressure_entry"] .pressure-grid {
        gap: 5px;
        margin: 0;
      }

      .stage.is-prologue-active[data-experience-phase="pressure_entry"] .pressure-row {
        grid-template-columns: 38px minmax(0, 1fr) 24px;
        gap: 6px;
        font-size: 10px;
      }

      .stage.is-prologue-active[data-experience-phase="standpoint_choice"] .pressure-director-panel {
        top: auto;
        bottom: calc(env(safe-area-inset-bottom) + 174px);
        max-height: 38vh;
        overflow: auto;
      ...
      .stage.is-prologue-active .scene-actions .primary {
        width: 100%;
        min-height: 44px;
      }

      .stage.is-prologue-active .scene-actions .ghost {
        display: none;
      }
...
<body>
  <main class="stage" data-testid="immersive-yantie-scene" data-experience-track="main-path" data-completion-status="active">
    ...
    <aside id="postCourtExplorer" class="chapter-map-panel post-court-explorer" aria-label="退朝后深度探索" hidden></aside>
  ...
  <script>
    ...
    const curatedExternalEchoItems = [
      {
        title: "校注传统与文本细读",
        provider: "王利器《盐铁论校注》",
        summary: "王利器的校注传统提醒用户：进入盐铁会议不能只看现代概念，也要回到篇章、训诂、异文和前人注释，理解每一句争论在文本中的位置。",
        viewpoint: "可借用的观点是：先把《盐铁论》作为需要细读和校勘的经典文本来处理，再谈制度史、思想史或当代问题。",
        copyright_note: "只展示书目、用途和原创摘要，不复制校注正文或注释内容。",
        source_boundary: "中国学者研究摘要"
      },
      {
        title: "思想史中的义利与治道",
        provider: "侯外庐、徐复观等中国思想史研究传统",
        summary: "中国思想史研究常把盐铁之议放在儒法、义利、王道与现实政治之间理解，强调它不只是财政政策争论，也是汉代思想秩序变化的一个窗口。",
        viewpoint: "可借用的观点是：贤良文学和桑弘羊的分歧，不应只化约为道德对利益，而要放在汉代政治思想和国家治理语言的转换中看。",
        copyright_note: "只展示研究史定位和原创概括，不引用现代专著正文。",
        source_boundary: "中国学者研究摘要"
      },
      {
        title: "桑弘羊与财政国家的再评价",
        provider: "吴慧《桑弘羊研究》等中国经济史研究",
        summary: "中国经济史研究中，对桑弘羊的评价并不只停留在“聚敛”或“与民争利”，也会追问边防、财政、产业控制和国家能力之间的现实关系。",
        viewpoint: "可借用的观点是：理解桑弘羊，既要看到财政官的国家责任，也要看到这种制度安排对民间经济和社会负担造成的压力。",
        copyright_note: "只展示作者、研究方向和观点摘要，不交付现代研究全文。",
        source_boundary: "中国学者研究摘要"
      },
      {
        title: "政治史背景中的盐铁会议",
        provider: "西嶋定生等日本汉代史研究",
        summary: "日本汉代史研究重视盐铁会议背后的政治结构、昭宣之际的权力关系和武帝政策遗产，能帮助用户理解霍光沉默和会议结果为何同样重要。",
        viewpoint: "可借用的观点是：盐铁会议并不是孤立的财政辩论，它也嵌在昭帝朝政治权力和政策继承的背景中。",
        copyright_note: "只展示研究取向和原创摘要，不复制论文或译注内容。",
        source_boundary: "日本学者研究摘要"
      },
      {
        title: "东亚译注中的经济论争",
        provider: "佐藤武敏《塩鉄論：漢代の経済論争》等日译注传统",
        summary: "日语译注传统常把《盐铁论》作为汉代经济论争来呈现，凸显盐、铁、商贸和财政制度如何成为理解帝国经济的入口。",
        viewpoint: "可借用的观点是：退朝后继续探索时，可以把《盐铁论》看作东亚学术共同阅读的制度文本，而不是单一语境下的中文古籍。",
        copyright_note: "只展示译注传统和观点摘要，不复制现代译文或注释。",
        source_boundary: "日本学者研究摘要"
      },
      {
        title: "《盐铁论》作为辩论文献",
        provider: "Esson M. Gale, The Discourses on Salt and Iron",
        summary: "Gale 的英译和导言使《盐铁论》更容易被英语世界作为一部围绕国家财政、产业控制和士人批评展开的辩论文献来阅读。",
        viewpoint: "可借用的观点不是某一句译文，而是阅读方法：把文本当作经过整理的朝廷论辩材料，既看政策争点，也看发言立场如何被组织。",
        copyright_note: "只展示书目与观点定位，不复制现代译文或导言原文。",
        source_boundary: "现代研究摘要"
      },
      {
        title: "昭宣时期的政策分歧",
        provider: "Michael Loewe, Crisis and Conflict in Han China",
        summary: "Loewe 对武帝以后至王莽以前政治危机和政策分歧的研究，可帮助用户把盐铁会议放入昭宣之际的财政压力、政治调整与士人意见之中。",
        viewpoint: "可借用的观点是：盐铁之议不只是道德和财政的二分，也关系到武帝遗产如何被继承、修正或约束。",
        copyright_note: "只作研究史定位，不引用专著正文。",
        source_boundary: "现代研究摘要"
      },
      {
        title: "铁业、官营与技术财政",
        provider: "Donald B. Wagner, The State and the Iron Industry in Han China",
        summary: "Wagner 从汉代铁业、官营组织和技术材料出发，帮助用户理解“铁官”并不只是抽象财政名词，而是资源、生产、技术和国家组织能力的结合。",
        viewpoint: "可借用的观点是：讨论盐铁官营时，需要同时看财政收入、生产组织和技术管理，而不仅是市场自由或国家垄断的口号。",
        copyright_note: "只展示研究方向和原创摘要，不复制现代研究原文。",
        source_boundary: "现代研究摘要"
      },
      {
        title: "帝国资源动员与制度化统治",
        provider: "Mark Edward Lewis, The Early Chinese Empires: Qin and Han",
        summary: "Lewis 对秦汉帝国形成和制度化统治的综述，有助于把盐铁、均输和平准理解为帝国资源动员的一部分，而不是孤立政策。",
        viewpoint: "可借用的观点是：国家能力、边防、交通和行政组织共同塑造了盐铁会议的制度背景。",
        copyright_note: "只展示书目卡和观点概括，不交付现代著作全文。",
        source_boundary: "现代研究摘要"
      },
      {
        title: "后世目录学如何看《盐铁论》",
        provider: "《四库全书总目提要》等目录传统",
        summary: "后世评价通常把它读作政论文献和思想争辩文本，而不是简单的会议实录。这个角度能帮助用户理解文本为何既有史料价值，也有编排和议论色彩。",
        viewpoint: "可借用的观点是：退朝后读《盐铁论》，应同时区分会议事实、文本整理和后世评价。",
        copyright_note: "目录学材料只作后世阅读线索，不改写会议事实。",
        source_boundary: "后世评说摘要"
      },
      {
        title: "国家、市场、民生的当代回声",
        provider: "退朝后问题定位",
        summary: "盐铁会议留下的不是一个可直接套用的答案，而是一组仍会反复出现的判断：公共支出从哪里来，民众承受多少，权力如何被约束。",
        viewpoint: "可借用的观点是：当代讨论只能作为反思入口，不能倒推古代会议事实，也不进入史证链。",
        copyright_note: "只展示原创问题摘要，不抓取或复制平台内容。",
        source_boundary: "当代讨论边界"
      }
    ];
    ...
    const materialGuideEntries = [
      {
        ...
        "question": "这场会议在编年史中如何留下轮廓？",
        "whatToRead": [
          "始元六年，诏问贤良文学，说明会议是在昭帝政治调整中发生。",
          "罢榷酤而盐铁如旧，说明退朝结果是有限调整，不是制度总废。",
          "编年材料可用来校准时间线、参与者和政策结果。"
        ],
        "howItHelps": "帮助用户把退朝案牍放回真实历史结果：争论被记录下来，但盐铁制度并未因此完全取消。",
        "entryNote": "适合从“全文争点”或退朝案牍回看，不替代《盐铁论》本身。",
        "details": [
      ...
      {
        "id": "institutional_background",
        "label": "制度背景",
        "source": "《汉书·食货志》《史记·平准书》",
        "status": "已入包 20 条",
        "defaultEntry": "退朝后深度探索；少量可作为主线轻提示",
        "question": "盐铁、均输、平准和酒榷为什么会成为朝议焦点？",
        "whatToRead": [
          "盐铁官营：国家直接控制关键资源收益。",
          "均输、平准：通过调运和平抑市场服务财政与供给。",
          "酒榷、算缗、告缗、边费：补足武帝以来财政动员和民间负担背景。"
        ],
        "howItHelps": "把“国家要钱”和“与民争利”都放进制度背景中看，避免把桑弘羊写成单纯聚敛，也避免把贤良文学写成空泛道德批评。",
        "entryNote": "材料导览展示制度地图；具体争论仍回到史证抽屉和全文争点。",
        "details": [
      ...
      {
        "id": "textual_reception",
        "label": "文本与后世评价",
        "source": "《四库全书总目提要》",
        "status": "已入包 8 条",
        "defaultEntry": "退朝后深度探索",
        "question": "《盐铁论》到底是会议实录、政论文本，还是后世整理？",
        "whatToRead": [
          "桓宽和《盐铁论》成书性质：文本保留会议争论，也带有整理和著述立场。",
          "目录学评价：帮助理解后世如何把它归入政论和思想争辩传统。",
          "文本性质材料：提醒用户区分会议事实、文本编排和后世阅读。"
        ],
        "howItHelps": "让用户知道自己看到的不是现场录音，而是一部经过保留、整理和解释的政治辩论文本。",
        "entryNote": "适合在退朝后阅读，用来校准证据可信度和文本边界。",
        "details": [
      ...
      },
      {
        ...
        "question": "为什么《黄帝四经》能帮助理解汉初黄老，而不能当作会议事实？",
        "whatToRead": [
          "道生法、法度、名实：显示黄老并非不要制度，而是强调制度应合乎秩序和名实。",
          "省苛事、毋夺民时：帮助理解国家动员何时越过民力边界。",
          "至静者圣：为权力节制和不轻动提供思想语言。"
        ],
        "howItHelps": "材料导览说明黄老材料从哪里来；哲学透镜再把这些材料用于解释制度、民力和权力节制。",
        "entryNote": "如需具体体验，进入“思想透镜：黄老”；本处只说明材料地图和边界。",
        "details": [
      ...
      },
      {
        ...
        "question": "贤良文学的道德批判为什么不是孤立的个人意见？",
        "whatToRead": [
          "《汉书》中的董仲舒、武帝文治和官学材料：说明经学逐步成为国家正当性语言。",
          "《春秋繁露》候选短摘：用于德刑、天人、王道和政治名分透镜。",
          "经学语境材料：解释道义批评如何进入制度化表达。"
        ],
        "howItHelps": "帮助用户理解贤良文学为什么能以义、利、德、刑等语言质疑财政政策，而不只是表达民间不满。",
        "entryNote": "如需具体体验，进入“思想透镜：经学语境”；材料导览只说明来源和用途。",
        "details": [
      ...
      },
      {
        ...
        "question": "现代研究能帮助用户继续问什么，而不是替用户给结论？",
        "whatToRead": [
          "王利器：从校勘、训释和注释传统回到《盐铁论》文本细节。",
          "侯外庐、徐复观等：中国思想史视野下的义利、儒法与治道问题。",
          "吴慧等：中国经济史视野下的桑弘羊、财政国家与民生负担。",
          "西嶋定生、佐藤武敏等：日本汉代史和日译注传统中的政治史、经济论争视角。",
          "Esson M. Gale：把《盐铁论》作为可比较的辩论文献来读。",
          "Michael Loewe：从昭宣时期政治危机和政策分歧理解盐铁会议。",
          "Donald B. Wagner：从汉代铁业、技术和国家工业控制理解铁官背景。",
          "Mark Edward Lewis：从秦汉帝国资源动员和制度化统治理解财政国家。"
        ],
        "howItHelps": "现代研究不进入史证链，但能帮助用户把盐铁会议放进研究史、制度史和思想史问题中继续阅读。",
        "entryNote": "具体作者和观点摘要集中放在“后世与当代讨论”；这里仅说明材料范围。",
        "details": [
          "只展示书目、主题标签和原创观点摘要，方便继续研究。",
          "补入中国校注、思想史、经济史和日本汉代史、译注传统，避免退朝后研究史只呈现欧美视角。",
          "不复制论文、专著或现代译注的长段内容。",
    ...
    const postCourtActions = [
      {
        id: "chapter-map",
        label: "全文争点",
        description: "打开六十篇争点图谱，只读史实证据。",
        exploreType: "historical_evidence"
      },
      {
        id: "philosophy-lens",
        label: "思想透镜",
        description: "从儒法、黄老、经学和制度国家角度重看争论。",
        exploreType: "philosophy_lens"
      },
      {
        id: "material-guide",
        label: "材料导览",
        description: "查看补充材料的来源、具体线索和使用边界。",
        exploreType: "material_guide"
      },
      {
        id: "power-network",
        label: "朝堂权力结构",
        description: "回到霍光沉默与权力边界一幕。",
        exploreType: "power_relation"
      },
      {
        id: "external-echo",
        label: "后世与当代讨论",
        description: "查看后世评说与当代问题的摘要；只作延伸思考。",
        exploreType: "external_echo"
      }
    ];
    ...
    const acceptanceScenarios = [
      {
        id: "opening-map",
        label: "Opening map and frontier pressure",
        phase: "pressure_entry",
        pressureIndex: 0,
        viewportTags: ["desktop", "mobile"],
        failLevel: "blocking"
      },
      {
        id: "first-choice",
        label: "First pressure choice",
        phase: "pressure_entry",
        pressureIndex: "last",
        viewportTags: ["desktop", "mobile"],
        failLevel: "blocking"
      ...
      {
        id: "standpoint-entry",
        label: "Generated standpoint before court",
        phase: "standpoint_choice",
        pressureIndex: "last",
        standpoint: "fiscal-official",
        viewportTags: ["desktop", "mobile"],
        failLevel: "major"
      },
      {
      ...
      {
        id: "fiscal-livelihood",
        label: "Fiscal and livelihood conflict",
        sceneActKey: "act-livelihood",
        phase: "court_debate",
        standpoint: "local-household",
        viewportTags: ["desktop", "mobile"],
        failLevel: "major"
      },
      {
        id: "key-evidence",
        label: "Key evidence drawer",
        sceneActKey: "act-livelihood",
        phase: "court_debate",
        standpoint: "local-household",
        openKeyEvidence: true,
        viewportTags: ["desktop", "mobile"],
        failLevel: "blocking"
      },
      {
        id: "power-silence",
        label: "Huo Guang silence",
        sceneActKey: "act-power",
        phase: "power_reveal",
        standpoint: "frontier-general",
        viewportTags: ["desktop", "mobile"],
        failLevel: "manual_review"
      },
      {
        id: "retirement-dossier",
        label: "Retirement dossier",
        sceneActKey: "judgment",
        phase: "judgment",
        standpoint: "fiscal-official",
        completed: true,
        viewportTags: ["desktop", "mobile"],
      ...
      {
        id: "post-court-explorer",
        label: "Post-court exploration entrance",
        sceneActKey: "judgment",
        phase: "judgment",
        standpoint: "fiscal-official",
        completed: true,
        expandPostCourtExplorer: true,
        viewportTags: ["desktop", "mobile"],
        failLevel: "major"
      }
    ];
    ...
    function sceneIndexForAcceptanceScenario(scenario) {
      if (!scenario.sceneActKey) return 0;
      if (scenario.sceneActKey === "judgment") {
        return scenes.findIndex(scene => scene.key === "judgment");
      }
      return scenes.findIndex(scene => scene.actKey === scenario.sceneActKey || scene.key === scenario.sceneActKey);
    }

    function seedAcceptanceTrajectory(scenario) {
      if (state.stanceTrajectory.length) return;
      const role = standpointRoles.find(item => item.id === (scenario.standpoint || "fiscal-official"));
      if (role) {
      ...
      experienceDirector.recordTrajectory({
        ...
        value: 50,
        note: "Stable screenshot acceptance entry; not a user judgment."
      });
    }

    function waitForAcceptancePaint() {
      return new Promise(resolve => {
        requestAnimationFrame(() => requestAnimationFrame(() => setTimeout(resolve, 120)));
      });
    }

    async function applyAcceptanceScenarioFromQuery() {
      const scenarioId = new URLSearchParams(window.location.search).get("acceptance");
      if (!scenarioId) return;
      const scenario = acceptanceScenarios.find(item => item.id === scenarioId);
      if (!scenario) return;

      closeChapterMap();
      closeEvidence();
      const stage = document.querySelector(".stage");
      delete stage.dataset.acceptanceScenario;
      document.getElementById("historyBoundary").textContent =
        `Acceptance scenario: ${scenario.label}. Failure level: ${scenario.failLevel}.`;

      state.completionStatus = scenario.completed ? "completed" : "active";
      state.userStandpoint = scenario.standpoint || state.userStandpoint || null;
      state.pressureIndex = scenario.pressureIndex === "last"
        ? pressureTimeline.length - 1
        : Number(scenario.pressureIndex || 0);

      if (scenario.phase === "pressure_entry" || scenario.phase === "standpoint_choice") {
        state.sceneIndex = 0;
        state.experiencePhase = scenario.phase;
        if (scenario.standpoint) seedAcceptanceTrajectory(scenario);
        await renderExperience();
      } else {
        const sceneIndex = sceneIndexForAcceptanceScenario(scenario);
        if (sceneIndex >= 0) state.sceneIndex = sceneIndex;
        state.experiencePhase = scenario.phase || experienceDirector.phaseForScene(scenes[state.sceneIndex]);
        seedAcceptanceTrajectory(scenario);
        await renderScene();
      }

      if (scenario.openKeyEvidence) {
        await openEvidence(primaryEvidenceIdsForScene(currentScene())[0]);
      }
      if (scenario.expandPostCourtExplorer) {
        openPostCourtExplorer();
        syncMainExperienceControls();
      }
      await waitForAcceptancePaint();
      stage.dataset.acceptanceScenario = scenario.id;
    }
    ...
    function syncMainExperienceControls() {
      ...
      chapterButton.textContent = postCourtUnlocked ? "退朝后探索" : "退朝后开放";
      const postCourtExplorer = document.getElementById("postCourtExplorer");
      if (postCourtExplorer) {
        if (!postCourtUnlocked) {
          closePostCourtExplorer();
        }
    ...
    async function boot() {
      ...
      renderChapterMap();
      await renderExperience();
      await applyAcceptanceScenarioFromQuery();
      startCanvas();
      updateSoundButton();
    }
    ...
    function renderDramaOverlay(scene) {
      ...
      syncMobileMapViewport();
    }

    function syncMobileMapViewport() {
      const svg = document.getElementById("hanMapScene");
      const isMobile = window.matchMedia("(max-width: 640px)").matches;
      const isPrologue = document.querySelector(".stage")?.classList.contains("is-prologue-active");
      svg.setAttribute("preserveAspectRatio", "xMidYMid meet");
      svg.dataset.mobileMapPriority = isMobile && isPrologue ? "true" : "false";
      svg.dataset.mobileMapMode = isMobile && isPrologue ? "contain" : "standard";
    }
    ...
    function renderPostCourtExplorer(activeAction = "overview") {
      const panel = document.getElementById("postCourtExplorer");
      if (!panel) return;
      panel.innerHTML = `
        <div class="chapter-map-head">
          <h2 class="chapter-map-title">退朝后探索</h2>
          <button class="chapter-map-close" type="button" data-close-post-court aria-label="关闭退朝后探索" title="关闭退朝后探索">×</button>
        </div>
        <div class="chapter-grid post-court-hub-grid" aria-label="退朝后探索入口">
          <section class="juan-band">
            <div class="juan-title">退朝之后</div>
            <p class="post-court-hub-copy">从这里继续进入全文争点、思想透镜、材料导览和后世讨论。它们都是退朝后的深度探索，不会改写你的案牍。</p>
            <div class="chapter-node-grid">
              ${postCourtActions.map(action => `
                <button class="chapter-node ${action.id === activeAction ? "is-active" : ""}" type="button"
                  data-post-court-action="${escapeHtml(action.id)}"
                  data-explore-type="${escapeHtml(action.exploreType)}"
                  ${action.id === "external-echo" ? `data-echo-enabled="${isExternalEchoEnabled() ? "live" : "curated"}"` : ""}>
        ...
        <aside class="chapter-detail" id="postCourtDetail">
          <div id="postCourtOverview" class="post-court-overview">
            <strong>先从全文争点进入，也可以换一个角度继续看。</strong>
            <ul>
              <li>全文争点负责回到《盐铁论》六十篇的冲突脉络。</li>
              <li>思想透镜、材料导览和后世讨论只在退朝后开放，不进入主体验。</li>
              <li>这里所有延伸内容都不会改写会议事实，也不会自动写入你的退朝案牍。</li>
            </ul>
          </div>
    ...
    function openPostCourtExplorer(activeAction = "overview") {
      if (!isPostCourtUnlocked()) {
        document.getElementById("historyBoundary").textContent = "退朝后探索将在退朝案牍后开放；当前主线只保留关键证据。";
        syncMainExperienceControls();
        return;
      }
      closeChapterMap();
      renderPostCourtExplorer(activeAction);
      const panel = document.getElementById("postCourtExplorer");
      panel.hidden = false;
      panel.classList.add("is-open");
      panel.setAttribute("aria-hidden", "false");
      document.getElementById("chapterMapScrim").classList.add("is-open");
      document.getElementById("historyBoundary").textContent = "退朝后探索从全文争点进入；思想透镜、材料导览和后世讨论都只作退朝后延伸。";
      pulseSound("evidence");
    }

    function closePostCourtExplorer() {
      const panel = document.getElementById("postCourtExplorer");
      if (!panel) return;
      panel.classList.remove("is-open");
      panel.hidden = true;
      panel.setAttribute("aria-hidden", "true");
      if (!state.chapterMapOpen) document.getElementById("chapterMapScrim").classList.remove("is-open");
    }

    function setActivePostCourtAction(actionId) {
      document.querySelectorAll("#postCourtExplorer [data-post-court-action]").forEach(node => {
        node.classList.toggle("is-active", node.dataset.postCourtAction === actionId);
      });
    }

    function hidePostCourtExplorationPanels(except = null) {
      const overview = document.getElementById("postCourtOverview");
      const lensPanel = document.getElementById("philosophyLensPanel");
      const externalPanel = document.getElementById("externalEchoPanel");
      const materialPanel = document.getElementById("materialGuidePanel");
      if (overview && except !== "overview") overview.hidden = true;
      if (lensPanel && except !== "philosophy") lensPanel.hidden = true;
      if (externalPanel && except !== "external") externalPanel.hidden = true;
      if (materialPanel && except !== "material") materialPanel.hidden = true;
    }

    function renderMaterialGuidePanel() {
      const panel = document.getElementById("materialGuidePanel");
      if (!panel) return;
      panel.hidden = false;
      panel.innerHTML = `
        <strong>退朝后材料导览</strong>
        <span>这里不是只标边界，而是告诉你每类材料具体看什么、能解释什么、下一步从哪里进入。它们不会自动进入你的退朝案牍，也不会改写会议事实。</span>
        <div class="material-guide-grid" aria-label="补充材料分层">
          ${materialGuideEntries.map(entry => `
            <section class="material-guide-card" data-material-guide-id="${escapeHtml(entry.id)}">
              ...
              <div class="material-guide-meta">
                <span>${escapeHtml(entry.status)}</span>
                <span>${escapeHtml(entry.defaultEntry)}</span>
              </div>
              <p class="material-guide-question">${escapeHtml(entry.question)}</p>
              <div class="material-guide-section">
                <b>具体看什么</b>
                <ul>
                  ${(entry.whatToRead || entry.details || []).map(detail => `<li>${escapeHtml(detail)}</li>`).join("")}
                </ul>
              </div>
              <div class="material-guide-section">
                <b>能解释什么</b>
                <p>${escapeHtml(entry.howItHelps || "")}</p>
              </div>
              <div class="material-guide-section">
                <b>下一步</b>
                <p>${escapeHtml(entry.entryNote || entry.defaultEntry)}</p>
              </div>
              <small>边界：${escapeHtml(entry.boundary)}</small>
            </section>
          `).join("")}
        </div>
    ...
    function openMaterialGuide() {
      hidePostCourtExplorationPanels("material");
      renderMaterialGuidePanel();
      document.getElementById("historyBoundary").textContent = "材料导览展示具体材料、对应问题和进入路径；仍不把解释材料变成会议事实。";
      pulseSound("evidence");
    }
    ...
    function renderExternalEchoPanel(message = null) {
      ...
      panel.innerHTML = `
        ...
        <span>这里展示后世评说、现代研究和当代问题的观点摘要。可以列作者和观点，但不复制现代论文或专著全文；这些内容也不会写入你的案牍。</span>
        ${items.length ? `
          <ul>
            ${items.map(item => `
              <li>
                ...
                ${item.provider ? `<small class="external-echo-source">${escapeHtml(item.provider)}</small>` : ""}
                ${item.summary ? `<span>${escapeHtml(item.summary)}</span>` : ""}
                ${item.viewpoint ? `<div class="external-echo-viewpoint">${escapeHtml(item.viewpoint)}</div>` : ""}
                ${item.copyright_note ? `<span class="external-echo-note">${escapeHtml(item.copyright_note)}</span>` : ""}
                <small>${escapeHtml(externalBoundaryLabel(item.source_boundary))}</small>
              </li>
            `).join("")}
          </ul>
        ` : `<span>暂未返回可展示条目。</span>`}
    ...
    async function openExternalEcho() {
      if (!isExternalEchoEnabled()) {
        ...
        document.getElementById("historyBoundary").textContent = "后世与当代讨论展示作者、书目和观点摘要；不复制现代研究全文，也不进入史证链。";
    ...
    document.getElementById("chapterMapToggle").addEventListener("click", () => openPostCourtExplorer());

    document.getElementById("postCourtExplorer").addEventListener("click", async event => {
      if (event.target.closest("[data-close-post-court]")) {
        closePostCourtExplorer();
        return;
      }
      ...
      setActivePostCourtAction(action);
    ...
    document.getElementById("chapterMapScrim").addEventListener("click", () => {
      closeChapterMap();
      closePostCourtExplorer();
    });
    ...
    function startCanvas() {
      ...
      window.addEventListener("resize", syncMobileMapViewport);
      resize();
