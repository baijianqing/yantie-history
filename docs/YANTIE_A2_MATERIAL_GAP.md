...
- 本轮涉及 evidence pack 的改动仅限 `YT-A2-MAT-006C` 经学透镜入包、`YT-A2-MAT-007` 制度人物 Actor 化和 `YT-A2-MAT-008` 酒榷制度背景补强；涉及 UI 的改动仅限 `YT-A2-MAT-006D`、`YT-A2-MAT-006E` 的退朝后只读入口；涉及验收脚本的改动仅限 `YT-A2-MAT-006F`；不修改 Schema、API、搜索逻辑、迁移或运行态数据。
