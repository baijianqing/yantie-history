...
- `YT-A2-MAT-012` 执行后，Playwright 静态验收已覆盖退朝后思想透镜入口、黄老分组、经学语境分组和透镜证据按钮，desktop、mobile、reduced-motion 三种视口均纳入检查。
- `YT-A2-MAT-013` 执行后，本材料门禁文档补齐 `YT-A2-MAT-012` 思想透镜验收完成状态；本轮不修改 evidence pack、UI 或验收脚本。
- `YT-A2-MAT-014` 执行后，Playwright 静态验收已覆盖退朝后外部回声入口的静态禁用状态、`external_echo` 边界提示和“不进入史证链”提示，desktop、mobile、reduced-motion 三种视口均纳入检查。
- `YT-A2-MAT-015` 执行后，本材料门禁文档补齐 `YT-A2-MAT-014` 外部回声边界验收完成状态；本轮不修改 evidence pack、UI、验收脚本或外部 adapter。
...
| 思想史 | 已有哲学透镜；`YT-A2-MAT-005B` 已补入 6 条《黄帝四经》黄老透镜；`YT-A2-MAT-005C` 已完成退朝后黄老分组；`YT-A2-MAT-006C` 已补入 6 条《汉书》/《春秋繁露》经学语境透镜；`YT-A2-MAT-006D` 至 `YT-A2-MAT-006F` 已完成退朝后经学分组、材料导览和验收覆盖；`YT-A2-MAT-012` 已完成退朝后思想透镜静态验收覆盖 | 如需进入主线轻提示，需另开任务验收；当前仍默认留在退朝后 | 解释儒法、黄老、经学、义利、国家能力和政治正当性等思想来源 | 退朝后思想透镜；退朝后材料导览 | 不直接支撑会议事实 |
| 后世评说 | 《四库提要》已完成首轮补强，`YT-A2-MAT-004` 已建立现代研究书目卡和版权边界；`YT-A2-MAT-014` 已完成外部回声静态边界验收 | 历代序跋仍可后续补充；现代研究如需入包需另开契约任务 | 作为退朝后延伸阅读和研究史定位；当代回声只作退朝后外部讨论 | 退朝后延伸阅读 | 通常不支撑会议事实；可支撑接受史或研究史说明 |
...
| P1 | 汉初至武帝思想语境 | 黄老与《黄帝四经》已完成计划冻结、候选短摘核验、首轮入包、退朝后黄老分组和思想透镜静态验收；经学、董仲舒和财政国家语境已完成计划冻结、《汉书》候选短摘核验、《春秋繁露》候选短摘核验、首轮入包、退朝后分组和材料导览验收 | 6 条黄老 EvidenceUnit、6 条经学语境 EvidenceUnit、黄老材料计划、经学语境计划与核验文档、退朝后思想透镜分组、材料导览入口和 Playwright 验收 |
| P2 | 现代研究书目卡 | 已完成首轮书目卡与版权边界；后续如需入包另开任务 | `docs/YANTIE_A2_MODERN_RESEARCH_INDEX.md` 7 张书目卡 |
| P2 | 当代回声 | 已完成静态禁用与 `external_echo` 边界验收；只作退朝后反思，不作历史证据 | `external_echo` 候选清单与 `external-echo-boundary` 验收场景 |
...
| `YT-A2-MAT-012` | 退朝后思想透镜验收场景覆盖 | 用 Playwright 验证思想透镜入口、黄老/经学分组和透镜按钮可见 | 退朝后验收矩阵 |
| `YT-A2-MAT-013` | 思想透镜验收状态收口 | 补登 012 思想透镜静态验收的完成状态 | 材料门禁文档 |
| `YT-A2-MAT-014` | 退朝后外部回声边界验收场景 | 用 Playwright 验证静态版外部回声入口禁用和边界提示 | 退朝后验收矩阵 |
| `YT-A2-MAT-015` | 外部回声验收状态收口 | 补登 014 外部回声边界验收的完成状态 | 材料门禁文档 |
...
- `YT-A2-MAT-012` 新增 `philosophy-lens` Playwright 验收场景，并确认退朝后思想透镜入口、黄老分组、经学语境分组和透镜按钮在三种视口下可见。
- `YT-A2-MAT-013` 补齐 `YT-A2-MAT-012` 在本材料门禁文档中的完成状态，并用材料文档测试守住该状态。
- `YT-A2-MAT-014` 新增 `external-echo-boundary` Playwright 验收场景，并确认静态版当代回声入口默认禁用、标记 `external_echo` 且不进入史证链。
- `YT-A2-MAT-015` 补齐 `YT-A2-MAT-014` 在本材料门禁文档中的完成状态，并用材料文档测试守住该状态。
- 在 `docs/YANTIE_TASK_BREAKDOWN.md` 登记 A2 首批任务卡。
- 本轮涉及 evidence pack 的改动仅限 `YT-A2-MAT-006C` 经学透镜入包、`YT-A2-MAT-007` 制度人物 Actor 化、`YT-A2-MAT-008` 酒榷制度背景补强和 `YT-A2-MAT-009` 贤良文学身份归并；涉及 UI 的改动仅限 `YT-A2-MAT-005C`、`YT-A2-MAT-006D`、`YT-A2-MAT-006E` 的退朝后只读入口；涉及验收脚本的改动仅限 `YT-A2-MAT-006F`、`YT-A2-MAT-012` 和 `YT-A2-MAT-014`；`YT-A2-MAT-010`、`YT-A2-MAT-011`、`YT-A2-MAT-013` 和 `YT-A2-MAT-015` 只新增文档测试和门禁说明；不修改 Schema、API、搜索逻辑、迁移或运行态数据。
