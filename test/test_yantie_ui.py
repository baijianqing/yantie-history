...
class YantieWebUiTests(unittest.TestCase):
    ...
    def test_yantie_a1_post_court_explorer_is_gated_and_degraded(self) -> None:
        ...
        self.assertIn("王利器《盐铁论校注》", html)
        self.assertIn("侯外庐、徐复观等中国思想史研究传统", html)
        self.assertIn("吴慧《桑弘羊研究》等中国经济史研究", html)
        self.assertIn("西嶋定生等日本汉代史研究", html)
        self.assertIn("佐藤武敏《塩鉄論：漢代の経済論争》等日译注传统", html)
        self.assertIn("中国学者研究摘要", html)
        self.assertIn("日本学者研究摘要", html)
        self.assertIn("Esson M. Gale, The Discourses on Salt and Iron", html)
        self.assertIn("Michael Loewe, Crisis and Conflict in Han China", html)
        self.assertIn("Donald B. Wagner, The State and the Iron Industry in Han China", html)
        self.assertIn("Mark Edward Lewis, The Early Chinese Empires: Qin and Han", html)
        self.assertIn("external-echo-viewpoint", html)
        self.assertIn("copyright_note", html)
        self.assertIn("现代研究摘要", html)
        ...
        self.assertIn("材料导览展示具体材料、对应问题和进入路径", html)
    ...
    def test_yantie_a2_post_court_material_guide_is_bounded(self) -> None:
        ...
        self.assertIn("具体看什么", html)
        self.assertIn("能解释什么", html)
        self.assertIn("下一步", html)
        self.assertIn("whatToRead", html)
        self.assertIn("howItHelps", html)
        self.assertIn("entryNote", html)
        ...
        self.assertIn("国家直接控制关键资源收益", entries_by_id["institutional_background"]["whatToRead"][0])
        self.assertIn("道生法", entries_by_id["huang_lao_lens"]["details"][0])
        self.assertIn("《黄帝四经》", entries_by_id["huang_lao_lens"]["question"])
        self.assertIn("现代研究", entries_by_id["modern_research"]["question"])
        self.assertIn("中西日书目卡与观点定位", entries_by_id["modern_research"]["source"])
        self.assertIn("王利器", entries_by_id["modern_research"]["whatToRead"][0])
        self.assertIn("侯外庐", entries_by_id["modern_research"]["whatToRead"][1])
        self.assertIn("西嶋定生", entries_by_id["modern_research"]["whatToRead"][3])
        self.assertIn("Michael Loewe", entries_by_id["modern_research"]["whatToRead"][5])
        self.assertIn("不复制论文", entries_by_id["modern_research"]["details"][2])
        for entry in entries:
            ...
            self.assertGreaterEqual(len(entry["whatToRead"]), 3)
            self.assertTrue(entry["howItHelps"])
            self.assertTrue(entry["entryNote"])
            self.assertGreaterEqual(len(entry["details"]), 3)

        self.assertIn("每类材料具体看什么", html)
    ...
    def test_yantie_a1_acceptance_runner_tracks_declared_scenarios(self) -> None:
        ...
        self.assertIn("material guide lacks concrete sections", script)
        ...
        self.assertIn("external echo lacks viewpoint cards", script)
